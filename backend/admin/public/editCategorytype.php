...
$typeData = $categorytypes->findType($_GET["id"]);
...
<main class="container">
    ...
    <div class="my-3 p-3 rounded shadow-sm section-div">
        <h6 class="border-bottom pb-2 mb-0 fw-bold">TIPA PĀRVALDĪBAS PANELIS</h6>
        <div class="my-3 text-center">
            <form class="w-75 row g-3 mt-3 mx-auto needs-validation" action="<?=BACKEND_DIR?>/handlers/updateHandlers/updateCategorytype.php" method="POST" novalidate>
                <div class="col-lg-8">
                    <div class="input-group has-validation">
                        <span class="input-group-text font-title">NOSAUKUMS</span>
                        <input hidden name="TypeID" value="<?= $typeData->TypeID; ?>"/>
                        <input id="TypeName" name="TypeName" minlength="1" maxlength="255" data-bs-toggle="popover" type="text" autocomplete="off" value='<?= $typeData->TypeName; ?>' class="form-control font-default" placeholder="Tips (piem., Tautas deju kolektīvs vai Koris)" required/>
                        <div class="invalid-feedback text-start">
                            Pārliecinies, vai ievadīji tipu pareizi! (min garums 1, max garums 255)
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
