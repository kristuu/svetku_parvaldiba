...
$participCollective = new ParticipCollectives();
$pcData = $participCollective->getParticipCollective($_GET['id'])[0];
...
<main class="container">
    ...
    <div class="my-3 p-3 rounded shadow-sm section-div">
        <h6 class="border-bottom pb-2 mb-0 fw-bold">DALĪBNIEKA SAISTĪJUMA AR KOLEKTĪVU DATU PĀRVALDĪBAS PANELIS</h6>
        <div class="my-3 text-center">
            <form class="w-75 row g-3 mt-3 mx-auto needs-validation" action="<?=BACKEND_DIR?>/handlers/updateHandlers/updateParticipCollective.php" method="POST" novalidate>
                <input hidden name="ID" value="<?= $pcData->ID; ?>"/>
                <div class="col-lg-6">
                    <div class="input-group has-validation">
                        ...
                        <select class="form-select font-default" id="ParticipantID" name="ParticipantID">
                            ...
                            foreach ($participants as $object) {
                                echo "<option class='font-default' value='$object->ParticipantID' ".(($object->ParticipantID === $pcData->ParticipantID) ? 'selected' : '').">$object->FName $object->LName</option>";
                            }
                            ?>
                        </select>
                    </div>
